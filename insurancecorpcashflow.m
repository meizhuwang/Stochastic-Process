...
while T(index)<T_max
  ...
  elseif 0<x && x<c %premium, claim payed at X(t) not c
    ...
    T(index+1)=T(index)+tau;
    u=rand;
    if u<(lambda/(lambda+alpha))   % premium
      X(index+1)=x+1;
    else            % claim
      X(index+1)=0;
    end
...
X_0=200;N=200;r=0.04;lambda=N;
alpha=r*N;beta=4;X_r=200;X_max=300;T_max=5;
d=30;c=20;
[X,t]=cashflow2(X_0,lambda,alpha,beta,c,d,X_r,X_max,T_max);
...
% Range A:R(1,1)=-lambda;R(1,2:c+1)=alpha;
% Range B:
for i=2:c
  R(i,i-1)=lambda;
  R(i,i+c)=alpha;
